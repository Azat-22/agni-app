...
const TeachersPage: FC<PropsWithChildren> = () => {
  const userListData = useAppSelector(selectListData);
   const filteredTeachersListData = userListData?.filter((item) => item.role.name === "Преподаватель")
  return (
    <AdminPageLayout>
      <AdminListTable userListData={filteredTeachersListData} />
    </AdminPageLayout>
  );
};
